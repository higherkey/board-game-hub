...
@Component({
    ...
    <div class="w-100 text-center">
      ...
      <!-- Phase: Clue Giving -->
      <div *ngIf="gameData?.phase === 0" class="card bg-surface shadow-md border-0 p-4 mb-4">
          <h3 class="mb-3 text-primary">Round {{ room?.roundNumber }}</h3>
          <p class="lead text-secondary">Target Word is defined! (Hidden from Guesser)</p>
          <div class="alert alert-info border-0 bg-info bg-opacity-10 text-primary fw-bold">
              Waiting for players to write their clues...
          </div>
          <div class="clue-counts mt-3">
             <h5 class="text-secondary">Clues Submitted: {{ getClueCount() }} / {{ getNonGuesserCount() }}</h5>
          </div>
          <!-- Show who is the guesser -->
          <div class="mt-4">
             <span class="badge bg-accent fs-5">Guesser: {{ getGuesserName() }}</span>
          </div>
      </div>

      <!-- Phase: Guessing / Elimination -->
      <div *ngIf="gameData?.phase === 1" class="card bg-surface shadow-md border-0 p-4 mb-4">
          <h3 class="mb-3 text-primary">Compare Clues</h3>
          <p class="text-secondary">Duplicate clues have been eliminated!</p>
          
          <div class="row justify-content-center gap-3 mt-4">
              <!-- Valid Clues -->
              <div *ngFor="let clue of getValidClues()" class="col-auto">
                  <div class="card bg-base border-0 shadow-sm p-3" style="min-width: 150px;">
                      ...
                      <ng-template #textClue>
                          <h4 class="m-0 text-primary fw-bold">{{ clue.value }}</h4>
                      </ng-template>
                      <small class="text-secondary mt-2 d-block">{{ clue.player }}</small>
                  </div>
              </div>
          ...
          <div class="mt-5">
              <h4 class="text-secondary">Waiting for <span class="text-accent fw-bold">{{ getGuesserName() }}</span> to guess...</h4>
          </div>
      </div>
      ...
      <div *ngIf="gameData?.phase === 2" class="card bg-surface shadow-md border-0 p-4 mb-4">
          ...
          <h4 class="text-secondary">Guesser answered: {{ gameData?.guesserResponse }}</h4>
      </div>
    </div>
  `
})
